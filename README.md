...
A GNOME Shell panel widget showing Claude and Codex usage side by side.

The panel shows each provider's icon and primary-window percentage:

```text
[Claude icon] 42%  [Codex icon] 18%
```
...
To use your own artwork, drop `claude.svg`, `codex.svg` (or `.png` / `.webp`) into `~/.config/ai-usage-widget/`, or pick a file in the extension's preferences. Custom icons keep their own colours.
...
- Refreshes both providers every two minutes, configurable in preferences
